...
import { getDishes, deleteDishById } from 'apiSdk/dishes';
import { DishInterface } from 'interfaces/dish';

type ColumnType = ColumnDef<DishInterface, unknown>;
...
export function DishListPage(props: DishListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DishInterface>>(
    () => `/dishes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DishInterface) => {
    if (hasAccess('dish', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/dishes/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/dishes/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('dish', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/dishes/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('dish', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="dish" />
        </Flex>

        {hasAccess('dish', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/dishes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
