...
export function MenuListPage(props: MenuListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMenus({
        relations: ['restaurant', 'dish.count'],
    ...
    hasAccess('restaurant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            ...
            >
              {record.restaurant?.description}
            </Link>
          ),
        }
      : null,
    hasAccess('dish', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'dish',
          header: 'Dish',
          accessorKey: 'dish',
          cell: ({ row: { original: record } }: any) => record?._count?.dish,
        }
      : null,
