...
import { DishListPage as ShowCaseList } from 'pages/dishes';

function DashboardPage() {
  const { session } = useSession();

  const dishStat = useSWR<PaginatedInterface<DishInterface>>(
    () => `/dishes`,
    () => getDishes({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Dishes',
      result: dishStat?.data?.totalCount,
      isLoading: dishStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`dishes`} passHref legacyBehavior>
                  <Button
